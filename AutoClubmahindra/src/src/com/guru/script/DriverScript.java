...
import src.com.guru.XL.Xls_Reader;
...
public class DriverScript {
	...
	public static By elementObject;
	...
	public static void main(String[] args) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, IOException, NoSuchMethodException, SecurityException {
		...
		FileInputStream fs = new FileInputStream(System.getProperty("user.dir")+"//src//src//com//guru//config//config.properties");
		CONFIG= new Properties();
		CONFIG.load(fs);
		
		fs = new FileInputStream(System.getProperty("user.dir")+"//src//src//com//guru//config//or.properties");
		OR= new Properties();
		OR.load(fs);
	...
	public void start() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException{
		...
		// Read Data from class XL Reader using XLS Reader Method
		
			suiteXLS = new Xls_Reader(System.getProperty("user.dir")+"//src//src//com//guru//excelfiles//Suite.xls");
		...
		for(currentSuiteID=2;currentSuiteID<=suiteXLS.getRowCount(Constants.TEST_SUITE_SHEET);currentSuiteID++){
			...
			if(suiteXLS.getCellData(Constants.TEST_SUITE_SHEET, Constants.RUNMODE, currentSuiteID).equals(Constants.RUNMODE_YES)){
				...
				currentTestSuiteXLS=new Xls_Reader(System.getProperty("user.dir")+"//src//src//com//guru//excelfiles//"+currentTestSuite+".xls");
		...
		for(currentTestStepID=2;currentTestStepID<=currentTestSuiteXLS.getRowCount(Constants.TEST_STEPS_SHEET);currentTestStepID++){
		  ...
		  if(currentTestCaseName.equals(currentTestSuiteXLS.getCellData(Constants.TEST_STEPS_SHEET, Constants.TCID, currentTestStepID))){
			...
			else if(data.startsWith(Constants.CONFIG))
			{
				...
				// Read Keyword Coloumn from Test Steps Sheet & Store in CurrentKeyword variable
	...
	public String getObject(String keyObject){
		String locator=keyObject.split("##")[0];
		return locator;
	}
	
	public By getObjectType(String keyObject){
		String locatorType=keyObject.split("##")[1];
		if(locatorType.equalsIgnoreCase("name")){
			elementObject=By.name(actualLocator);
		}
		else if(locatorType.equalsIgnoreCase("id")){
			elementObject=By.id(actualLocator);
		}
		else if(locatorType.equalsIgnoreCase("xpath")){
			elementObject=By.xpath(actualLocator);
		}
		else if(locatorType.equalsIgnoreCase("class")){
			elementObject=By.className(actualLocator);
		}
		else if(locatorType.equalsIgnoreCase("tag")){
			elementObject=By.tagName(actualLocator);
		}
		else if(locatorType.equalsIgnoreCase("linkText")){
			elementObject=By.linkText(actualLocator);
		}
		else if(locatorType.equalsIgnoreCase("partialLink")){
			elementObject=By.partialLinkText(actualLocator);
		}
		else if(locatorType.equalsIgnoreCase("css")){
			elementObject=By.cssSelector(actualLocator);
		}
		else{
